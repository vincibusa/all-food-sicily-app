...
export const RestaurantListCard: React.FC<RestaurantListCardProps> = ({
  item,
  onPress,
  animationProps,
}) => {
  ...
  const formatCuisineTypes = (cuisineTypes?: string[]) => {
    ...
    return cuisineTypes.slice(0, 2).map(cuisine => 
      cuisine.charAt(0).toUpperCase() + cuisine.slice(1).replace('-', ' ')
    );
  };

  const cuisineTypesFormatted = formatCuisineTypes(item.cuisine_type);
    ...
    <Animated.View style={styles.container} {...animationProps}>
      ...
      >
        ...
        <View style={styles.restaurantInfo}>
          ...
          {/* Cuisine types tags */}
          {cuisineTypesFormatted.length > 0 && (
            <View style={styles.tagsContainer}>
              {cuisineTypesFormatted.map((cuisine, index) => (
